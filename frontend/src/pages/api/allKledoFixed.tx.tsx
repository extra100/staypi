...
const StockSelectorTable = () => {
  ...
  const { loading, idContact } = useIdContact(selectedContactGroup || null)
  ...
  const handleSave = () => {
    ...
    const invoiceData = {
      ...
      items: dataSource.map((item) => ({
        ...
        discount_amount: item.gapPriceTotal || 0, // Total gap price (gapPrice * qty)

        finance_account_id: item.finance_account_id,
        discount_percent: item.selectedDiscountValue || 0, // Use the discount percentage value from the item
