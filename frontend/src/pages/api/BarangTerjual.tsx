...
export function useBarangTerjual(date: string, warehouseIds: number) {
  const [loading, setLoading] = useState(true)
  const [warehouseStock, setWarehouseStock] = useState<WarehouseStock[]>([])
  const cacheKey = `${date}-${warehouseIds}`
  const cache = new Map<string, WarehouseStock[]>()

  const fetchBarangTerjual = async (page: number, perPage: number) => {
    const response = await fetch(
      `${HOST}/reportings/salesPerProduct?date=${date}&warehouse_ids=${warehouseIds}&page=${page}&per_page=${perPage}`,
      {
        headers: {
  ...
  const fetchAllWarehouseStock = async (perPage: number) => {
    ...
    const firstPageResult = await fetchBarangTerjual(page, perPage)
    ...
    for (let i = 2; i <= totalPages; i += batchSize) {
      ...
      for (let j = i; j < i + batchSize && j <= totalPages; j++) {
        requests.push(fetchBarangTerjual(j, perPage))
      }
