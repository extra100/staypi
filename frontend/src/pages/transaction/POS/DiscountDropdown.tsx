...
const DiskonDropdowns = () => {
  ...
  const renderDiscountColumns = (price: number) =>
    discountRates.map((rate) => (
      <span key={rate.label} style={{ flex: 1 }}>
        {calculateDiscount(price, rate.percentage).toFixed(2)}
      </span>
    ))
          ...
          .map((product) => (
            ...
            >
              ...
              >
                ...
                {renderDiscountColumns(product.price)}
              </div>
            </Select.Option>
          ))}
      </Select>
...
export default DiskonDropdowns
