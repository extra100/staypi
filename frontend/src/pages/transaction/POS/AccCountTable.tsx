...
const Pos = () => {
    ...
    setDataSource((prev) => [
      ...
      ...selectedItems
        ...
        .map((item) => ({
          ...
          selectedDiscount: 'Retail 10%', // Default discount
          subtotal: item.price, // Default subtotal
        })),
    ])
  ...
  const calculateTotalSubtotal = () => {
    return dataSource.reduce((total, item) => total + item.subtotal, 0)
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'subtotal',
      render: (text: number) => (
        <div style={{ textAlign: 'center' }}>{text.toFixed(2)}</div>
      ),
    },
  ]
  ...
  return (
    <div className="my-select-container">
      ...
      >
        Total Subtotal: {calculateTotalSubtotal().toFixed(2)}
      </div>
