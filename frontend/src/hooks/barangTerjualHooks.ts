...
export const useGetBartersQuery = () =>
  useQuery({
    queryKey: ['barters'],
    queryFn: async () => {
      try {
...
export const useGetThenAddBartersQuery = (batchSize: number, offset: number) =>
  useQuery({
    queryKey: ['barters', batchSize, offset],
    queryFn: async () => {
      try {
        const response = await apiClient.get<{
          data: {
          ...
          meta: { total: number }
        }>(`/api/barters/barters?limit=${batchSize}&offset=${offset}`)
...
export const useAddBarter = () => {
  const queryClient = useQueryClient()
  return useMutation(
    (barangter: Barter) => {
      return apiClient.post<Barter>(`/api/barters`, barangter)
    },
    {
      onSuccess: () => {
